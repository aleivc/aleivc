...
const Tools = () => {
    ...
    const handleAction = (action, item) => {
        ...
        // const pubUrl = tempPubUrl;
        const temp = list.map((f) => {
            return f.deviceId === item.deviceId
                ? { ...item, loading: true, action }
                : { ...f };
        });
        // 设置对应的 record
        setList([...temp]);
        ...
        <div>
            ...
            <div className="flex gap-1" style={{ width: 320 }}>
                <Input
                    ...
                    onChange={(e) => onChange(e, "deviceId")}
                />
            ...
            <div className="flex flex-wrap gap-2">
                {list.map((item, index) => {
                    return (
                        <Badge.Ribbon text={index + 1} placement="start" key={index}>
                            <Card
                                style={{
                                    width: 320,
                                }}
                                actions={[
                                    ...
                                    >
                                        <Spin spinning={item.loading && item.action === "open"}>
                                            <Text>开阀</Text>
                                        </Spin>
                                    </div>,
                                    <div onClick={() => handleAction("close", item, index)}>
                                        <Spin spinning={item.loading && item.action === "close"}>
                                            <Text>关阀</Text>
                                        </Spin>
                                    </div>,
                                    <div onClick={() => handleAction("stop", item, index)}>
                                        <Spin spinning={item.loading && item.action === "stop"}>
                                            <Text>停止</Text>
                                        </Spin>
                                    </div>,
                            ...
                            >
                                <Meta
                                    description={
                                        <div className="flex">
                                            ...
                                            <Paragraph copyable>{item.deviceId}</Paragraph>
                                        </div>
                                    }
                                />
                                <br />
                                ...
                                <Spin spinning={item.initLoading}>
                                    ...
                                    <div className="text-center">
                                        当前阀位 {item.properties["cur_pos"]}% / 目标阀位 - %
                                    </div>
                                    <br />
                                    <div className="flex flex-wrap gap-1">
                                        <Tag
                                            color={item.properties["all_close"] === 1 ? "green" : ""}
                                        >
                                            关到位
                                        </Tag>
                                        <Tag
                                            color={item.properties["all_open"] === 1 ? "green" : ""}
                                        >
                                            开到位
                                        </Tag>
                                        <Tag
                                            color={
                                                item.properties["control_mode"] === 2 ? "" : "green"
                                            }
                                        >
                                            {item.properties["control_mode"] === 0 ? "就地" : "远程"}
                                        </Tag>
                                        <Tag
                                            color={
                                                item.properties["value_trouble"] === 0 ? "warning" : ""
                                            }
                                        >
                                            故障
                                        </Tag>
                                    ...
                                    <div className="flex">
                                        <Input
                                            value={value.op_pos}
                                            onChange={(e) => onChange(e, "op_pos")}
                                        />
                                        <Button
                                            onClick={() => handleAction("set_pos", item, index)}
                                        >
                                            设定开度
                                        </Button>
                                    </div>
